...
class ModelRegistry:
    ...
    def _migrate_existing_models(self):
        ...
        try:
            ...
            self._save_registry()
            print(f"Migration completed: processed {len(self.models)} models across {len(model_groups)} model families")
    ...
    def _get_model_family(self, model_id: str) -> List[ModelMeta]:
        ...
        family = []
        for model in self.models.values():
            if (model.id == root_model_id or 
                model.parent_model_id == root_model_id or 
                self._is_descendant(model.id, root_model_id)):
                family.append(model)
    ...
    def save_model(self, model_data: bytes, filename: str, metadata: ModelMetaCreate) -> str:
        ...
        if metadata.is_new_version and metadata.parent_model_id:
            ...
            existing_models = [m for m in self.models.values() if m.name == metadata.name]
            if existing_models:
        ...
        try:
            ...
            try:
                ...
                if hasattr(loaded_model, 'feature_names_in_'):
                    full_metadata.feature_names = loaded_model.feature_names_in_.tolist()
