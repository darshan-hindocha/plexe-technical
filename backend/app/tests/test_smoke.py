    ...
    def test_model_preview_regressor(self, test_regressor_file):
        ...
        with open(test_regressor_file, "rb") as f:
            response = client.post(
                "/api/v1/models/preview",
                files={
                    "file": ("test_regressor.joblib", f, "application/octet-stream")
                },
            )
        assert response.status_code == 200
        data = response.json()
...
class TestModelPrediction:
    ...
    def test_model_prediction(self, uploaded_model_id):
        ...
        response = client.post(
            f"/api/v1/models/{uploaded_model_id}/predict", json=prediction_data
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_prediction_nonexistent_model(self):
        ...
        prediction_data = {"features": {"feature_0": 1.0}}

        response = client.post(
            "/api/v1/models/nonexistent-id/predict", json=prediction_data
        )
...
class TestChatSystem:
    ...
    def test_chat_without_api_keys(self, mock_chat_agent):
        ...
        response = client.post(
            "/api/v1/models/chat", json={"message": "Hello, can you help me?"}
        )
...
class TestErrorHandling:
    ...
    def test_malformed_prediction_request(self):
        ...
        response = client.post(
            "/api/v1/models/some-id/predict", json={"invalid": "structure"}
        )
