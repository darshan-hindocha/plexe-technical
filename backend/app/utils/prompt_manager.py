...
class PromptManager:
    ...
    def get_prompt_templates(
        ...
        try:
            ...
            logger.warning(
                f"Base template {base_template_name} not found, using empty base"
            )
            base_template = {}
        ...
        try:
            ...
            logger.warning(
                f"Override template {override_template_name} not found, using base only"
            )
            return base_template
...
def get_prompt_templates(
    ...
    template_dict = manager.get_prompt_templates(
        base_template_name, override_template_name
    )
    ...
    return PromptTemplates(
        ...
        planning=PlanningPromptTemplate(
            initial_plan=planning_dict.get(
                "initial_plan", "Think through the problem step by step."
            ),
            update_plan_pre_messages=planning_dict.get(
                "update_plan_pre_messages", "Let's update the plan."
            ),
            update_plan_post_messages=planning_dict.get(
        ...
        final_answer=FinalAnswerPromptTemplate(
            pre_messages=final_answer_dict.get(
                "pre_messages", "Let me provide the final answer."
            ),
            post_messages=final_answer_dict.get(
                "post_messages", "This completes the task."
            ),
        ),
    )
