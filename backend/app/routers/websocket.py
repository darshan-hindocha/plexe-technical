...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, client_id: str) -> bool:
        ...
        logger.info(
            f"WebSocket connected: {client_id} (total: {self.connection_count})"
        )
        return True
    ...
    def disconnect(self, client_id: str):
        ...
        if client_id in self.active_connections:
            ...
            self.connection_count -= 1
            logger.info(
                f"WebSocket disconnected: {client_id} (total: {self.connection_count})"
            )
...
async def handle_chat_message(
    websocket: WebSocket, client_id: str, message_data: dict, chat_agent: ChatAgent
):
    ...
    try:
        ...
        await manager.send_personal_message(
            {"type": "typing", "timestamp": datetime.now().isoformat()}, client_id
        )
            ...
            try:
                ...
                if message_type == "chat":
                    await handle_chat_message(
                        websocket, client_id, message_data, chat_agent
                    )
