...
class ModelMetaCreate(ModelMetaBase):
    ...
    is_new_version: bool = Field(
        False, description="Whether this is a new version of an existing model"
    )
    parent_model_id: Optional[str] = Field(
        None, description="ID of the parent model for versioning"
    )
...
class ModelMeta(ModelMetaBase):
    ...
    feature_names: Optional[List[str]] = Field(
        None, description="Expected input features"
    )
    model_info: Optional[Dict[str, Any]] = Field(
        None, description="Additional model information"
    )
    ...
    is_latest: bool = Field(
        True, description="Whether this is the latest version of the model"
    )
